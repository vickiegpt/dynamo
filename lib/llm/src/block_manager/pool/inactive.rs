...
impl<T: Storage, M: BlockMetadata> BlockPoolInner<T, M> {
    ...
    pub fn return_block(&mut self, mut block: BlockType<T, M>) {
        ...
        // increment the available blocks
    ...
    fn match_hashes(&mut self, hashes: Vec<SequenceHash>) -> Vec<BlockType<T, M>> {
    ...
    pub fn acquire_next_free(&mut self) -> Option<BlockType<T, M>> {
        ...
        if let Some(key) = self.priority_set.pop_first() {
            tracing::trace!("acquired priority/registered block map");
...
//     return_rx: mpsc::UnboundedReceiver<BlockType<T, M>>,
