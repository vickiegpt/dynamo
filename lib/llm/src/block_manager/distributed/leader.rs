...
impl KvbmLeader {
    pub async fn new(mut config: KvbmLeaderConfig) -> anyhow::Result<Self> {
        ...
        leader.spawn_barrier_task(drt, leader_sockets.pub_url.clone(), leader_sockets.ack_url.clone());
        leader.spawn_zmq_task(leader_sockets, cancel_token);

        Ok(leader)
    }

    fn spawn_barrier_task(&self, drt: DistributedRuntime, leader_sockets_pub_url: String, leader_sockets_ack_url: String) {
        ...
        tokio::spawn(async move {
            match KvbmLeader::run_barrier_sync(
                ...
                Ok((num_device_blocks, num_host_blocks, num_disk_blocks)) => {
                    // write back results
                    state.num_device_blocks.store(num_device_blocks, Ordering::Release);
                    state.num_host_blocks.store(num_host_blocks, Ordering::Release);
                    state.num_disk_blocks.store(num_disk_blocks, Ordering::Release);
    ...
    async fn run_barrier_sync(
        ...
        let barrier_id_worker_to_leader = format!("{}{}", leader_config.barrier_id_prefix, "-worker-to-leader");
        tracing::info!(
        ...
        let bytes_per_block: usize = worker_data
            .values()
            .map(|d| d.bytes_per_block)
            .sum();

        assert!(bytes_per_block > 0, "bytes_per_block must be greater than 0");

        tracing::info!("Worker to leader barrier synced with {} workers", leader_config.world_size);
        tracing::debug!("Worker data: {:?}", worker_data);

        let num_host_blocks = compute_num_blocks(&leader_config.host_blocks_config, bytes_per_block);
        let num_disk_blocks = compute_num_blocks(&leader_config.disk_blocks_config, bytes_per_block);

        // Start the second sync to transfer num_host_blocks and num_disk_blocks to worker
        let barrier_id_leader_to_worker = format!("{}{}", leader_config.barrier_id_prefix, "-leader-to-worker");
        tracing::info!(
            "Syncing leader barrier with {} workers on barrier id {}",
            leader_config.world_size,
    ...
    fn spawn_zmq_task(&self, leader_sockets: LeaderSockets, cancel: tokio_util::sync::CancellationToken) {
        ...
        tokio::spawn(async move {
            ...
            match res {
                Ok(zmq) => {
                    let _ = cell.set(zmq);
                    // mark ready
                    state.workers_allocation_ready.store(true, Ordering::Release);
                }
                Err(e) => {
    ...
    pub fn spawn_leader_readiness_barrier(&self, drt: DistributedRuntime) {
        ...
        handle.spawn(async move {
            match KvbmLeader::run_leader_readiness(
                drt,
                leader_config,
            ).await {
                Ok(()) => {
    ...
    async fn run_leader_readiness(drt: DistributedRuntime, leader_config: KvbmLeaderConfig) -> anyhow::Result<()> {
        let barrier_id_leader_ready = format!("{}{}", leader_config.barrier_id_prefix, "-leader-ready");
        tracing::info!(
        ...
        let leader_readiness_barrier: LeaderBarrier<(), ()> =
            LeaderBarrier::new(
        ...
        let _ = leader_readiness_barrier
            .sync(&drt, &())
            .await
            .map_err(|e| anyhow::anyhow!("Failed to sync leader readiness barrier on leader: {:?}", e))?;
    ...
    pub async fn wait_worker_sync_ready(&self) -> bool {
        if self.is_worker_sync_ready() { return true; }
        if self.is_worker_sync_done()  { return false; }
