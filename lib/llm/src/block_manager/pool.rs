...
//     match_tx: mpsc::UnboundedSender<MatchRequest<S, M>>,
//     control_tx: mpsc::UnboundedSender<ControlRequest<S, M>>,
//     fence_tx: mpsc::UnboundedSender<oneshot::Sender<()>>,
//     return_handle: Arc<ReturnHandleImpl<S, M>>,
...
// struct ReturnHandleImpl<S: Storage, M: BlockMetadata> {
//     return_tx: mpsc::UnboundedSender<BlockType<S, M>>,
// }

// impl<S: Storage, M: BlockMetadata> ReturnHandle<Block<S, M>> for ReturnHandleImpl<S, M> {
//     fn return_to_pool(&self, value: BlockType<S, M>) {
...
#[derive(Default)]
pub struct BlockPoolInner<S: Storage, M: BlockMetadata> {
...
// #[derive(Dissolve)]
// pub struct MatchSingle<S: Storage, M: BlockMetadata> {
//     hash: SequenceHash,
//     return_handle: Arc<ReturnHandleImpl<S, M>>,
//     tx: oneshot::Sender<Option<UniqueBlock<S, M>>>,
// }

// #[derive(Dissolve)]
// pub struct MatchMultiple<S: Storage, M: BlockMetadata> {
//     hashes: Vec<SequenceHash>,
//     return_handle: Arc<ReturnHandleImpl<S, M>>,
//     tx: oneshot::Sender<Vec<UniqueBlock<S, M>>>,
// }

// #[derive(Dissolve)]
// pub struct Take<S: Storage, M: BlockMetadata> {
//     count: u32,
//     return_handle: Arc<ReturnHandleImpl<S, M>>,
//     tx: oneshot::Sender<Vec<UniqueBlock<S, M>>>,
// }

// pub enum MatchRequest<S: Storage, M: BlockMetadata> {
//     MatchSingle(MatchSingle<S, M>),
//     MatchMultiple(MatchMultiple<S, M>),
//     Take(Take<S, M>),
// }
...
#[derive(Dissolve)]
pub struct InsertControl<S: Storage, M: BlockMetadata> {
    block: Block<S, M>,
    tx: oneshot::Sender<()>,
}
...
pub enum ControlRequest<S: Storage, M: BlockMetadata> {
    Insert(InsertControl<S, M>),
