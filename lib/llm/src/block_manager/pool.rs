...
pub type UniqueBlock<S: Storage> = PoolItem<Block<S>>;

pub struct AvailableBlocks<T: Storage + Send + Sync + 'static> {
    match_tx: mpsc::UnboundedSender<MatchRequest<Block<T>>>,
    control_tx: mpsc::UnboundedSender<ControlRequest<Block<T>>>,
    fence_tx: mpsc::UnboundedSender<oneshot::Sender<()>>,
    return_handle: Arc<ReturnHandleImpl<Block<T>>>,
    total_blocks: Arc<AtomicU64>,
    available_blocks: Arc<AtomicU64>,
    join_handle: JoinHandle<()>,
}

impl<T: Storage + Send + Sync> AvailableBlocks<T> {
    ...
    pub async fn match_blocks(&self, hashes: Vec<SequenceHash>) -> Result<Vec<PoolItem<Block<T>>>> {
    ...
    pub async fn insert(&self, block: Block<T>) -> Result<()> {
        let (tx, rx) = oneshot::channel();
        if self
            .control_tx
...
struct ReturnHandleImpl<T: Storage + Send + Sync + 'static> {
    return_tx: mpsc::UnboundedSender<PoolValue<Block<T>>>,
}

impl<T: Storage + Send + Sync> ReturnHandle<Block<T>> for ReturnHandleImpl<T> {
    fn return_to_pool(&self, value: PoolValue<Block<T>>) {
...
impl<T: Storage + Send + Sync> From<&Block<T>> for PriorityKey {
...
#[derive(Default)]
struct AvailableBlocksState<T: Storage + Send + Sync + 'static> {
    // Direct lookup by sequence_hash
    lookup_map: HashMap<SequenceHash, PoolValue<Block<T>>>,

    // // Ordered by timestamp (oldest first)
    priority_set: BTreeMap<PriorityKey, SequenceHash>,

    // Fully Uninitialized
    uninitialized_set: VecDeque<PoolValue<Block<T>>>,
...
impl<T: Storage + Send + Sync> AvailableBlocksState<T> {
    ...
    fn insert(&mut self, block: PoolValue<Block<T>>) {
    ...
    fn match_hashes(
        ...
        return_handle: Arc<ReturnHandleImpl<T>>,
    ) -> Vec<PoolItem<Block<T>>> {
    ...
    fn handle_match_single(&mut self, match_single: MatchSingle<T>) {
    ...
    fn handle_match_multiple(&mut self, match_multiple: MatchMultiple<T>) {
    ...
    fn take(&mut self) -> Option<PoolValue<Block<T>>> {
    ...
    fn handle_take(&mut self, take: Take<T>) {
    ...
    fn handle_match_request(&mut self, match_request: MatchRequest<T>) {
        match match_request {
    ...
    fn handle_control_request(&mut self, control_request: ControlRequest<T>) {
    ...
    fn handle_insert(&mut self, block: Block<T>) {
        self.available_blocks
    ...
    fn handle_return(&mut self, block: PoolValue<Block<T>>) {
        self.available_blocks
            .fetch_add(1, std::sync::atomic::Ordering::SeqCst);
        self.return_tick += 1;
...
#[async_trait]
impl<T: Storage + Send + Sync> PoolExt<Block<T>> for AvailableBlocksState<T> {}

#[derive(Dissolve)]
pub struct MatchSingle<T: Storage + Send + Sync + 'static> {
    hash: SequenceHash,
    return_handle: Arc<ReturnHandleImpl<T>>,
    tx: oneshot::Sender<Option<UniqueBlock<T>>>,
}

#[derive(Dissolve)]
pub struct MatchMultiple<T: Storage + Send + Sync + 'static> {
    hashes: Vec<SequenceHash>,
    return_handle: Arc<ReturnHandleImpl<T>>,
    tx: oneshot::Sender<Vec<UniqueBlock<T>>>,
}

#[derive(Dissolve)]
pub struct Take<T: Storage + Send + Sync + 'static> {
    count: u32,
    return_handle: Arc<ReturnHandleImpl<T>>,
    tx: oneshot::Sender<Vec<UniqueBlock<T>>>,
}

pub enum MatchRequest<T: Storage + Send + Sync + 'static> {
    MatchSingle(MatchSingle<T>),
    MatchMultiple(MatchMultiple<T>),
    Take(Take<T>),
}
...
#[derive(Dissolve)]
pub struct InsertControl<T: Storage + Send + Sync + 'static> {
    block: Block<T>,
    tx: oneshot::Sender<()>,
}
...
pub enum ControlRequest<T: Storage + Send + Sync + 'static> {
    Insert(InsertControl<T>),
...
pub async fn progress_engine<T: Storage + Send + Sync + 'static>(
    match_rx: mpsc::UnboundedReceiver<MatchRequest<T>>,
    return_rx: mpsc::UnboundedReceiver<PoolValue<Block<T>>>,
    ctrl_rx: mpsc::UnboundedReceiver<ControlRequest<T>>,
...
pub(crate) mod tests {
    ...
    pub fn create_blocks(sequence: Tokens, block_size: usize) -> Vec<KvBlock<NullStorage>> {
