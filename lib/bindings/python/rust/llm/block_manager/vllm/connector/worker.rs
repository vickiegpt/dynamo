...
impl KvConnectorWorker {
    fn new(py_drt: PyDistributedRuntime, vllm_worker_id: String, rank: usize) -> anyhow::Result<Self> {
...
impl PyKvConnectorWorker {
    #[new]
    #[pyo3(signature = (py_drt, vllm_worker_id, rank))]
    pub fn new(py_drt: PyDistributedRuntime, vllm_worker_id: String, rank: usize) -> PyResult<Self> {
        let connector_worker: Box<dyn Worker> =
            Box::new(KvConnectorWorker::new(py_drt, vllm_worker_id, rank).map_err(to_pyerr)?);
