...
impl BlockManagerBuilder {
    ...
    pub fn worker_id(mut self, id: u64) -> Self { self.worker_id = id; self }
    pub fn page_size(mut self, ps: usize) -> Self { self.page_size = ps; self }
    pub fn leader(mut self, l: distributed::KvbmLeader) -> Self { self.leader = Some(l); self }
    pub fn disable_device_pool(mut self, yes: bool) -> Self { self.disable_device_pool = yes; self }
    ...
    pub async fn build(self) -> Result<BlockManager> {
        ...
        let leader = self
            .leader
            .ok_or_else(|| anyhow::anyhow!("leader is required (runtime is always taken from leader)"))?;
        ...
        let mut config = dynamo_llm::block_manager::KvBlockManagerConfig::builder().runtime(runtime_config);
        ...
        let inner = dynamo_llm::block_manager::KvBlockManager::<Logical<DistributedLeaderWorkerResources>, BasicMetadata>::new(
            config,
            resources,
        )
        .await?;
    ...
    /// Synchronous convenience (blocks on the runtime you supplied/derived).
    pub fn build_blocking(self) -> Result<BlockManager> {
        // Reuse the async build by entering a temporary future on the provided runtime.
        // We need a runtime already (from leader.dissolve() or .runtime()).
        // So we peek it first via a small dry-run to extract drt; simplest is to clone self and run build().
        // If you prefer zero async here, inline the same `block_on` logic directly like in `build()`.
