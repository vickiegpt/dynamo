...
static IDENTIFIER_REGEX: Lazy<Regex> = Lazy::new(|| Regex::new(r"^[a-z0-9_-]+$").unwrap());

/// Regex for path segment validation (includes dots for object names)
/// Allows: lowercase letters, digits, hyphens, underscores, dots
static PATH_SEGMENT_REGEX: Lazy<Regex> = Lazy::new(|| Regex::new(r"^[a-z0-9_.-]+$").unwrap());
...
pub(super) fn validate_namespace_segment(
    segment: &str,
    allow_internal: bool,
