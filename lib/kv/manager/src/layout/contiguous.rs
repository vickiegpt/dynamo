...
//!
//! This module provides the implementation for a block-continuous memory layout
//! of KV blocks.
//!
//! A single allocation of memory, minimally of size [nblocks, nlayers, page_size, inner_dim],
//! is required to
...
mod tests {
    ...
    use crate::storage::tests::{NullDeviceAllocator, NullDeviceStorage};
    use crate::storage::{StorageType, SystemAllocator};
    ...
    #[test]
    fn creation_insufficient_storage() {
        init_logging();
        let config = LayoutConfig {
            num_blocks: NUM_BLOCKS,
            num_layers: NUM_LAYERS,
