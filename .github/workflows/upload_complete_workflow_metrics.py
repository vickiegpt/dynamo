...
class CompleteWorkflowMetricsUploader:
    def __init__(self):
    ...
    def post_workflow_metrics(self) -> None:
        ...
        db_data["s_pipeline_id"] = str(self.run_id)
        db_data["s_commit_sha"] = workflow_data.get('head_sha', self.sha)
        db_data["s_gitlab_branch"] = workflow_data.get('head_branch', self.ref_name)
        db_data["s_pipeline_status"] = workflow_data.get('status', 'unknown')
        db_data["s_pipeline_source"] = workflow_data.get('event', self.event_name)
        ...
        db_data["s_user_alias"] = actor.get('login', self.actor)
        db_data["s_project_id"] = f"github_{self.repo.replace('/', '_')}"
        db_data["s_project_name"] = self.repo
        
        # Additional GitHub-specific fields
        db_data["s_workflow_name"] = workflow_data.get('name', self.workflow_name)
        db_data["s_github_event"] = self.event_name
        db_data["s_github_ref"] = self.ref
    ...
    def post_all_job_metrics(self) -> None:
        ...
        print(f"Successfully processed {jobs_processed} jobs")
    ...
    def post_single_job_metrics(self, job_data: Dict[str, Any]) -> None:
        ...
        # User and project info
        db_data["s_user_alias"] = self.actor
        db_data["s_project_id"] = f"github_{self.repo.replace('/', '_')}"
        db_data["s_project_name"] = self.repo
        
        # Additional GitHub-specific fields
        db_data["s_workflow_name"] = self.workflow_name
        db_data["s_github_event"] = self.event_name
        db_data["s_github_ref"] = self.ref
...
def main():
    ...
    try:
        uploader = CompleteWorkflowMetricsUploader()
    ...
    try:
        print("Uploading job metrics for all jobs in workflow...")
        uploader.post_all_job_metrics()
        print("All job metrics uploaded successfully")
    except Exception as e:
        error_msg = str(e)
