#!/usr/bin/env python3
"""
Enhanced script to upload GitHub Actions workflow and job metrics to the dynamo-metrics database.
This version can upload metrics for any workflow run, not just the current one.
It's designed to be called by the upload-metrics.yml workflow after other workflows complete.
"""
...
# Import utility functions from the original script
...
class WorkflowMetricsUploader:
    def __init__(self):
        ...
        self.pipeline_index = os.getenv('PIPELINE_INDEX', '')
        ...
        self.original_run_id = os.getenv('ORIGINAL_RUN_ID')
        self.original_repo = os.getenv('ORIGINAL_REPOSITORY')
        self.original_head_sha = os.getenv('ORIGINAL_HEAD_SHA')
        self.original_head_branch = os.getenv('ORIGINAL_HEAD_BRANCH')
        self.original_event = os.getenv('ORIGINAL_EVENT')
        self.original_actor = os.getenv('ORIGINAL_ACTOR')
        self.original_workflow_name = os.getenv('ORIGINAL_WORKFLOW_NAME')
        
        if not self.original_run_id or not self.original_repo:
            raise ValueError("Missing original workflow information")
    ...
    def post_workflow_metrics(self) -> None:
        """Extract and post workflow (pipeline equivalent) metrics for the original workflow"""
        print(f"Uploading metrics for workflow run {self.original_run_id} in {self.original_repo}")
        
        # Get workflow run data from GitHub API
        workflow_data = self.get_github_api_data(f"/repos/{self.original_repo}/actions/runs/{self.original_run_id}")
        ...
        db_data = {}
        db_data["_id"] = f"github_{self.original_run_id}_{self.original_repo.replace('/', '_')}"
        db_data["s_pipeline_id"] = str(self.original_run_id)
        db_data["s_commit_sha"] = workflow_data.get('head_sha', self.original_head_sha)
        db_data["s_gitlab_branch"] = workflow_data.get('head_branch', self.original_head_branch)
        db_data["s_pipeline_status"] = workflow_data.get('status', 'unknown')
        db_data["s_pipeline_source"] = workflow_data.get('event', self.original_event)
        ...
        db_data["s_user_alias"] = actor.get('login', self.original_actor)
        db_data["s_project_id"] = f"github_{self.original_repo.replace('/', '_')}"
        db_data["s_project_name"] = self.original_repo
        
        # Additional GitHub-specific fields
        db_data["s_workflow_name"] = workflow_data.get('name', self.original_workflow_name)
        db_data["s_github_event"] = self.original_event
        db_data["s_github_ref"] = workflow_data.get('head_branch', self.original_head_branch)
        
        self.post_to_db(self.pipeline_index, db_data)

    def post_all_job_metrics(self) -> None:
        """Extract and post metrics for all jobs in the original workflow"""
        print(f"Uploading job metrics for workflow run {self.original_run_id}")
        
        # Get jobs data from GitHub API
        jobs_data = self.get_github_api_data(f"/repos/{self.original_repo}/actions/runs/{self.original_run_id}/jobs")
    ...
    def post_single_job_metrics(self, job_data: Dict[str, Any]) -> None:
        ...
        db_data["_id"] = f"github_{job_id}_{self.original_repo.replace('/', '_')}"
        db_data["s_pipeline_id"] = str(self.original_run_id)
        db_data["s_pipeline_status"] = job_data.get('status', 'unknown')
        db_data["s_pipeline_source"] = self.original_event
        db_data["s_job_id"] = str(job_id)
        db_data["s_job_stage"] = "github_actions"  # GitHub Actions doesn't have stages like GitLab
        db_data["s_gitlab_branch"] = self.original_head_branch
        ...
        db_data["s_job_status"] = job_data.get('conclusion', job_data.get('status', 'unknown'))
        db_data["s_job_failure_reason"] = job_data.get('conclusion', '-') if job_data.get('conclusion') in ['failure', 'cancelled'] else '-'
        ...
        # User and project info
        db_data["s_user_alias"] = self.original_actor
        db_data["s_project_id"] = f"github_{self.original_repo.replace('/', '_')}"
        db_data["s_project_name"] = self.original_repo
        
        # Additional GitHub-specific fields
        db_data["s_workflow_name"] = self.original_workflow_name
        db_data["s_github_event"] = self.original_event
        db_data["s_github_ref"] = self.original_head_branch
        
        self.post_to_db(self.jobs_index, db_data)
        print(f"Uploaded metrics for job: {job_name}")

def main():
    """Main function to upload GitHub Actions metrics for a completed workflow"""
    try:
    ...
    print(f"Processing metrics for workflow '{uploader.original_workflow_name}' (run {uploader.original_run_id})")
