...
from utils.plot import plot_decode_3d_surface, plot_prefill_interpolation
...
def generate_prefill_benchmarks(args):
    ...
    for isl in range(
        ...
        if gap_result is not None:
            ...
            logger.info(
                f"ISL={isl}: TTFT={ttft:.2f}ms, Throughput={isl/ttft*1000:.2f} tokens/s"
            )
        else:
    ...
    if len(prefill_isl) > 2:
        ...
        result_file = os.path.join(
            args.artifact_dir, "prefill_interpolation_results.json"
        )
...
def generate_decode_benchmarks(args):
    ...
    for isl in range(
        ...
        if max_concurrency < 1:
            logger.warning(
                f"Skipping ISL={isl} as max_concurrency would be {max_concurrency}"
            )
            continue
        ...
        for num_request in sweep_num_request:
            ...
            logger.info(
                f"Running decode benchmark for ISL={isl}, OSL={osl}, num_request={num_request}"
            )
            genai_perf_artifact_dir = (
                f"{args.artifact_dir}/gap_isl{isl}_osl{osl}_n{num_request}"
            )
            ...
            if gap_result is not None:
                ...
                throughput_per_gpu = gap_result["output_token_throughput"][
                    "avg"
                ]  # Assuming single GPU
                ...
                logger.info(
                    f"ISL={isl}, OSL={osl}, n={num_request}: ITL={itl:.2f}ms, KV_usage={kv_usage:.2f}, Throughput={throughput_per_gpu:.2f} tokens/s"
                )
            else:
                logger.warning(
                    f"Benchmark failed for ISL={isl}, OSL={osl}, num_request={num_request}"
                )
    ...
    if len(x_kv_usage) > 2:
        ...
        result_file = os.path.join(
            args.artifact_dir, "decode_interpolation_results.json"
        )
        with open(result_file, "w") as f:
            json.dump(results, f, indent=2)
        logger.info(f"Interpolation results saved to {result_file}")
