...
> [!NOTE]
> For detailed information about Dynamo's metrics system, including hierarchical metrics, automatic labeling, and usage examples, see the [Metrics Guide](../../guides/metrics.md).
...
2. Web servers started. The ones that end in /metrics are in Prometheus format:
   ...
   - Dynamo components registered with the DistributedRuntime automatically expose metrics through the built-in MetricsRegistry. See the [MetricsRegistry guide](../../guides/metrics.md) for details on the available metrics endpoints.
   - Uncomment the appropriate lines in prometheus.yml to poll port 9091.
   - Start worker(s) that publishes KV Cache metrics: [service_metrics.md](../../runtime/metrics/service_metrics.md) can populate dummy KV Cache metrics.
...
![grafana image](../../../deploy/metrics/grafana-dynamo-composite.png)
