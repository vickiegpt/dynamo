...
You can enable [request migration](../../../guides/deployment/fault_tolerance/request_migration.md) to handle worker failures gracefully. Use the `--migration-limit` flag to specify how many times a request can be migrated to another worker:

```bash
python3 -m dynamo.sglang ... --migration-limit=3
```

This allows a request to be migrated up to 3 times before failing. See the [Request Migration Architecture](../../../guides/deployment/fault_tolerance/request_migration.md) documentation for details on how this works.
...
The `deploy/` folder contains Kubernetes Custom Resource Definition (CRD) templates for both single-node and multi-node deployments:

**Single-Node Configurations:**
- `agg.yaml` - Basic aggregated deployment with frontend and single worker
- `agg_router.yaml` - Aggregated deployment with KV cache routing capabilities  
- `disagg.yaml` - Disaggregated deployment with separated prefill and decode workers

**Multi-Node Support:**
All deployment patterns support multi-node configurations through the `multinode` section in resource specifications. This enables:
- Distribution of workloads across multiple physical nodes
- Scaling GPU resources beyond a single machine
- Support for large models requiring extensive tensor parallelism
