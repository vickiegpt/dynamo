...
def stream(name, state, prompt):
    ...
    with requests.post(
        GEN_URL, json=payload, headers=headers, stream=True, timeout=600
    ) as r:
        r.raise_for_status()
        printed_qid = False
...
def main():
    ...
    # Test validation
    a_finish_type = (
        A.get("finish_reason", {}).get("type") if A.get("finish_reason") else None
    )
    b_finish_type = (
        B.get("finish_reason", {}).get("type") if B.get("finish_reason") else None
    )
