...
CONFIG_FILE="/workspace/examples/llm/configs/disagg_router.yaml"
HOST="127.0.0.1"
...
dynamo serve graphs.disagg_router:Frontend -f $CONFIG_FILE &
    ...
    response=$(curl -s -X POST http://$HOST:$PORT/v1/chat/completions \
            ...
            -d '{
                ...
                "messages": [
                    {
                        "role": "user",
                        "content": "We will start with some tokens to actually trigger disaggregation. What is capital of France?"
                    }
                ],
                "seed": 1,
                "temperature": 0.0,
