...
def detect_result_type(log_dir: str) -> Optional[str]:
    ...
    for item in log_path.iterdir():
        ...
        if item.is_file() and item.name.startswith("client_") and item.name.endswith(".log.txt"):
            legacy_indicators += 1
...
def parse_test_results(
    ...
    if parser_type == "aiperf":
        ...
        from tests.fault_tolerance.deploy.legacy_parse_results import main as parse_legacy
    ...
    if info["type"] == "aiperf":
        ...
        for item in log_path.iterdir():
            if item.is_dir() and item.name.startswith("client_"):
                ...
                attempts = len([d for d in item.iterdir() if d.is_dir() and d.name.startswith("attempt_")])
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--log-paths",
        nargs="+",
        help="Multiple log directories to process"
    )
    parser.add_argument(
        "--format",
        choices=["fancy", "markdown"],
        default="fancy",
        help="Table format"
    )
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="SLA threshold for latency"
    )
    parser.add_argument(
        "--force-parser",
