...
async def init(runtime: DistributedRuntime, config: Config):
    ...
    os.environ["LMCACHE_CHUNK_SIZE"] = os.environ.get("LMCACHE_CHUNK_SIZE", "256")
    os.environ["LMCACHE_LOCAL_CPU"] = os.environ.get("LMCACHE_LOCAL_CPU", "True")
    os.environ["LMCACHE_MAX_LOCAL_CPU_SIZE"] = os.environ.get("LMCACHE_MAX_LOCAL_CPU_SIZE", "200")
    
    if "LMCACHE_LOCAL_DISK" in os.environ:
        os.environ["LMCACHE_LOCAL_DISK"] = os.environ["LMCACHE_LOCAL_DISK"]
        os.environ["LMCACHE_MAX_LOCAL_DISK_SIZE"] = os.environ.get("LMCACHE_MAX_LOCAL_DISK_SIZE", "10")


    if "LMCACHE_CONFIG_FILE" in os.environ:
        logger.info(f"Using LMCache config file: {os.environ['LMCACHE_CONFIG_FILE']}")
    else:
        logger.info("Using LMCache environment variable configuration")

    logger.info("LMCache is enabled with the following configuration:")
    logger.info(f"  - Chunk size: {os.environ.get('LMCACHE_CHUNK_SIZE', '256')}")
    logger.info(f"  - CPU offload: {os.environ.get('LMCACHE_LOCAL_CPU', 'True')}")
    logger.info(f"  - CPU size: {os.environ.get('LMCACHE_MAX_LOCAL_CPU_SIZE', '5')} GB")
    if "LMCACHE_LOCAL_DISK" in os.environ:
        logger.info(f"  - Disk offload: {os.environ.get('LMCACHE_LOCAL_DISK')}")
        logger.info(f"  - Disk size: {os.environ.get('LMCACHE_MAX_LOCAL_DISK_SIZE', '10')} GB")
