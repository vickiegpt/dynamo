...
func TestDockerSecretIndexer_RefreshIndex(t *testing.T) {
	...
	mockSecrets := []corev1.Secret{
		{
			...
			Data: map[string][]byte{
				".dockerconfigjson": []byte(`{"auths":{"docker.io":{}, "gitlab-master.nvidia.com:5005/registry1":{}}}`),
			},
		},
		{
			ObjectMeta: metav1.ObjectMeta{
			...
			Type: corev1.SecretTypeDockerConfigJson,
			Data: map[string][]byte{
				".dockerconfigjson": []byte(`{"auths":{"gitlab-master.nvidia.com:5005/registry2":{}}}`),
			},
		},
		{
			ObjectMeta: metav1.ObjectMeta{
	...
	secrets, err = i.GetSecrets("default", "gitlab-master.nvidia.com:5005")
	if err != nil {
		t.Errorf("DockerSecretIndexer.GetSecrets() error = %v, wantErr %v", err, nil)
	}
	if len(secrets) != 2 {
	...
	secrets, err = i.GetSecrets("another-namespace", "gitlab-master.nvidia.com:5005")
	if err != nil {
		t.Errorf("DockerSecretIndexer.GetSecrets() error = %v, wantErr %v", err, nil)
	}
	if len(secrets) != 1 {
