...
def generate_pydantic_model(name: str, properties: Dict[str, Any], required: List[str] = None) -> str:
    ...
    for prop_name, prop in properties.items():
        ...
        if prop_type == "object" and "properties" in prop:
            ...
            nested_model = generate_pydantic_model(nested_model_name, prop["properties"], prop.get("required", []))
        ...
        # Handle arrays
            ...
            if "properties" in items:
                ...
                nested_model = generate_pydantic_model(nested_model_name, items["properties"], items.get("required", []))
                fields.append(nested_model)
...
def main():
    # Read the CRD file
    crd_path = Path(__file__).parent.parent.parent.parent / "operator" / "config" / "crd" / "bases" / "nvidia.com_dynamodeployments.yaml"
    ...
    # Add TypeMeta model
    models.append("""class TypeMeta(BaseModel):
    api_version: str = Field(..., alias="apiVersion")
    kind: str = Field(...)
""")

    # Add ObjectMeta model
    models.append("""class ObjectMeta(BaseModel):
    name: str = Field(...)
    namespace: str = Field(...)
...
""")
    ...
    spec_model = generate_pydantic_model("DynamoDeploymentSpec", schema["properties"]["spec"]["properties"])
    models.append(spec_model)
    
    # Generate the main DynamoDeployment model
    models.append("""class DynamoDeployment(BaseModel):
