...
class BaseTensorrtLLMEngine:
    ...
    async def generate(self, request: TRTLLMWorkerRequest):
        ...
        try:
            # --- Prepare inputs dictionary based on request content ---
            worker_inputs = {}
            is_multimodal = False

            # 1. Add Text Tokens (Assuming always present)
            worker_inputs["prompt_token_ids"] = request.tokens.tokens
            logger.debug(f"Added {len(request.tokens.tokens)} text tokens to input.")

            # 2. Add Image Features if present
            if hasattr(request, 'image_features') and request.image_features is not None:
                logger.info("Image features found in request, adding to 'multi_modal_data'.")
                is_multimodal = True
                # Pass features directly (assuming they are List[List[float]] from worker)
                # The underlying engine (LLM class) should handle this structure.
                worker_inputs["multi_modal_data"] = {"image": request.image_features}
                # Log shape if possible (assuming list of lists)
                try:
                    shape_info = f"{len(request.image_features)}x{len(request.image_features[0])}" if request.image_features else "empty"
                    logger.debug(f"Added image features with shape approx {shape_info} to multi_modal_data.")
                except:
                    logger.debug("Added image features to multi_modal_data (shape unknown).")
            else:
                logger.info("No image features found, proceeding with text-only input.")
            ...
            # Handle remote prefill
            # WARNING: This logic still likely needs review for multimodal correctness.
            # Does prefill work on image features? How should inputs be handled after prefill?
            if self._remote_prefill and self._server_type == ServerType.GEN:
                logger.warning("Remote prefill logic might need adjustments for multimodal inputs.")
                ctx_response_obj = await self._get_remote_prefill_response(request)

                # Update disaggregated params only, leave worker_inputs prepared above as is?
                # This needs careful verification based on how remote prefill + multimodal is designed.
                disaggregated_params = (
                    DisaggregatedTypeConverter.to_llm_disaggregated_params(
                        DisaggregatedParams(
