...
import requests
from onnxruntime import OrtValue  # Import necessary classes
...
ONNX_MODEL_DIR = os.path.join(
    os.path.dirname(__file__), "..", "llava_onnx_encoder"
)  # Relative path
...
ORIGINAL_MODEL_PATH = (
    "/tmp/llava-1.5-7b-hf"  # Or path where original files are accessible
)
        ...
        if DEVICE == "cuda":
            ...
            if not os.path.exists(TRT_CACHE_PATH):
                try:
                    ...
                    logger.info(
                        f"Created TensorRT engine cache directory: {TRT_CACHE_PATH}"
                    )
                except OSError as e:
                    logger.error(
                        f"Failed to create TensorRT cache directory {TRT_CACHE_PATH}: {e}"
                    )
        ...
        try:
            ...
            if not os.path.exists(VISION_TOWER_ONNX_PATH):
                raise FileNotFoundError(
                    f"Vision tower ONNX model not found at {VISION_TOWER_ONNX_PATH}"
                )
            if not os.path.exists(PROJECTOR_ONNX_PATH):
                raise FileNotFoundError(
                    f"Projector ONNX model not found at {PROJECTOR_ONNX_PATH}"
                )
            ...
            self.vision_output_type = ONNX_TYPE_MAP.get(
                self.vision_output_meta.type, np.float32
            )
            self.proj_output_type = ONNX_TYPE_MAP.get(
    ...
    def encode_image_onnx(self, image_path_or_url: str) -> np.ndarray:
        ...
        try:
            ...
            preprocessed_image_ortvalue = OrtValue.ortvalue_from_numpy(
                preprocessed_image_np, ORT_DEVICE
            )
            logger.debug(
            ...
            logger.debug(
                f"Allocating vision output buffer with shape: {vision_output_shape} and type: {self.vision_output_type} on device: {ORT_DEVICE}"
            )

            vision_output_ortvalue = OrtValue.ortvalue_from_shape_and_type(
                vision_output_shape, self.vision_output_type, ORT_DEVICE
            )
            logger.debug(
            ...
            logger.debug(
                f"Allocating projector output buffer with shape: {proj_output_shape} and type: {self.proj_output_type} on device: {ORT_DEVICE}"
            )

            final_embeddings_ortvalue = OrtValue.ortvalue_from_shape_and_type(
                proj_output_shape, self.proj_output_type, ORT_DEVICE
            )
            logger.debug(
            ...
            logger.debug(
                f"Final embeddings NumPy shape: {final_embeddings_np.shape}, dtype: {final_embeddings_np.dtype}"
            )
