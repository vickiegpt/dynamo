...
async def test_local_connector(namespace: str, component: str = None):
    ...
    try:
        ...
        for comp in components:
            print(f"  - {comp['name']}: {comp['replicas']} replicas, status: {comp['status']}")
        ...
        if component:
            ...
            if replicas > 0:
                # Interactive scaling
                should_scale = input(f"\nDo you want to test scaling '{component}'? (y/n): ").lower() == 'y'
                if should_scale:
                    target = int(input(f"Enter target number of replicas for '{component}' (current: {replicas}): "))
                ...
                # Interactive restart
                should_restart = input(f"\nDo you want to test restarting '{component}'? (y/n): ").lower() == 'y'
                if should_restart:
...
async def main():
    parser = argparse.ArgumentParser(description="Test the CircusController and LocalConnector")
    ...
    parser.add_argument("--circus-only", action="store_true", help="Test only CircusController")
    parser.add_argument("--connector-only", action="store_true", help="Test only LocalConnector")
