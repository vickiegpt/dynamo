...
class VllmEncodeWorker:
    ...
    def sample_frames(self, video_path: str, num_frames: int = 8) -> List[np.ndarray]:
        ...
        try:
            ...
            if len(frames) != num_frames:
                logger.warning(
                    f"Could only sample {len(frames)} frames out of {num_frames} requested"
                )
    ...
    async def encode(self, request: EncodeRequest) -> AsyncIterator[EncodeResponse]:
        ...
        try:
            ...
            sampled_frames_np_list = self.sample_frames(
                request.image_url, num_frames=request.num_frames or 8
            )  # Use num_frames from request
