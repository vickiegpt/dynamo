...
def run_benchmark(args):
    ...
    command = (
        ...
        f"--tokenizer {args.tokenizer} --synthetic-input-tokens-mean {args.isl_unique} "
        f"--output-tokens-mean {args.osl} "
        ...
        f"--num-prefix-prompts 1 --prefix-prompt-length {args.isl_shared} "
    )
    ...
    LOGGER.info(
        f"ISL shared: {args.isl_shared}, ISL unique: {args.isl_unique}, OSL: {args.osl}, "
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Number of tokens in the input sequence that are shared (prefix)",
    )
    parser.add_argument(
        "--isl-unique",
        type=int,
        required=True,
        help="Number of tokens in the input sequence that are unique (suffix)",
    )
    parser.add_argument(
        "--osl",
        type=int,
